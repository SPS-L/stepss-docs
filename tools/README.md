...
## capture-gui.sh cannot currently reach the main window

**As of STEPSS 3.80 the `gui-*` figures cannot be regenerated by this script,
and the two `gui-ssa-results` figures were captured another way.** Two separate
faults, both silent:

- **The window geometry seed is dead.** `launch.sh` writes `windowMaximised`,
  `windowX`, `windowY`, `windowWidth` and `windowHeight` into the private
  preferences, and **nothing in the GUI reads any of them any more**: they
  survive only in `PreferenceMigration`'s key list and in two test files.
  `StepssUI` now calls `frame.setExtendedState(JFrame.MAXIMIZED_BOTH)`
  unconditionally on every launch, with a comment saying the geometry memory
  was removed on purpose. So the window fills the screen, every fixed
  coordinate in `capture-gui.sh` is off by the 60 px window origin and more,
  and the run stalls waiting for a dialog its click never opened.

  It cannot be forced from outside either. `xdotool` has no `windowstate` in
  the version here (3.20160805.1), `wmctrl` is not installed, and an openbox
  `<application><maximized>no</maximized>` rule loses, because the application
  maximises itself *after* the window is mapped. Fixing this properly means
  giving the GUI a geometry override it honours, which is a `stepss-java-ui`
  change, not one that can be made here.

- **`frame_geom` can crop the title bar off.** It assumes `xdotool
  getwindowgeometry` reports the *frame's* origin under a reparenting window
  manager. For the small-signal results window it reports (62,100) where
  `xwininfo` reports the client origin (61,80) with `_NET_FRAME_EXTENTS` of
  1,1,20,5, so cropping on xdotool's numbers starts below the title bar and
  takes a strip of desktop below the window to make up the height. `xwininfo`
  plus the frame extents is unambiguous and is what the replacement uses.

Until the first of those is fixed, a figure of a window the application opens
by itself can be captured by opening that window alone, which is what the two
`gui-ssa-results` figures were. Any such driver must go through
`StepssUI.installTheme` rather than calling FlatLaf itself: that method carries
`Table.rowHeight` 22 and the other metrics the figures depend on, and it must
also set `JFrame.setDefaultLookAndFeelDecorated(true)`, which is what gives the
themed title bar the rest of the set has.
