...
to be installed for this: earlier versions needed Gnuplot on the `PATH`, because
the engine drew those curves itself by piping to it, and from RAMSES v3.77 the
engine writes the file and calls nothing.
...
<Aside type="note">
`stepss.__runTimeObs__` is now always `True` and is deprecated. It used to
report whether Gnuplot had been found at import time, and `addRunObs()` was a
silent no-op when it was `False`. Do not branch on it.
</Aside>

<Aside type="caution">
Plotting *during* a run, refreshing as the file grows, is not built into this
package yet. STEPSS GUI does that natively; here the curve file is readable as
soon as the engine has flushed it, so a loop of your own can do the same.
</Aside>
