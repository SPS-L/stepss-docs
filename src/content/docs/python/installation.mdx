...
The file is plain text, with a `#` header naming the columns and time in column
...
`stepss.monitor` plots chosen quantities *while* the simulation runs, one panel
per observable:
...
[API reference](/python/api-reference/) lists the descriptor vocabulary and the
rest of the methods.
