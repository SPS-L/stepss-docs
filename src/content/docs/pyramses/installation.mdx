...
From `3.58` on, a PyRAMSES version names the RAMSES engine bundled in the
wheel: `pyramses 3.58` carries RAMSES 3.58. A fourth component appears when
PyRAMSES is released without a new RAMSES behind it, so `3.58.1` and `3.58.2`
also bundle RAMSES 3.58.

To pin an engine version, pin the matching PyRAMSES:

```bash
pip install "pyramses~=3.58.0"    # any 3.58.x, all bundling RAMSES 3.58
```
...
```python
import pyramses
print(pyramses.__version__, pyramses.__ramses_version__, pyramses.__helios_version__)
```
