...
- `S_or_A`: `S` (series-branch insertion) or `A` (shunt-style anchor) flag.
...
Three two-port models are available in every RAMSES distribution (both the standalone executable and the shared library used by PyRAMSES): `HVDC_LCC` / `twop_HVDC_LCC`, `DCL_WCL` / `twop_DCL_WCL`, and `HVDC_VSC_SC` / `twop_HVDC_VSC_SC`. Other two-port models documented below (`twop_HVDC_VSC`, the Hydro-Québec family `twop_CSVGN5`, `twop_CHENIER`, etc.) are not callable out of the box. They can be enabled by extending RAMSES through URAMSES (see the [URAMSES guide](/developer/uramses/)).
:::
