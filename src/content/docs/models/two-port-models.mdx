...
Three two-port models are available in every RAMSES distribution (both the standalone executable and the shared library used by PyRAMSES): `HVDC_LCC` / `twop_HVDC_LCC`, `DCL_WCL` / `twop_DCL_WCL`, and `HVDC_VSC_SC` / `twop_HVDC_VSC_SC`. `twop_HVDC_VSC`, also documented below, is compiled into the library but registered under no name; it becomes callable by adding one case to the URAMSES router (see the [URAMSES guide](/developer/uramses/)).
:::
...
### twop_HVDC_VSC: Voltage Source Converter HVDC (not registered)

:::caution
`twop_HVDC_VSC` is documented here for reference but is **not callable out of the box** in a standard RAMSES distribution. To use it, enable it via URAMSES (see the [URAMSES guide](/developer/uramses/)). For a built-in VSC-HVDC model, use `twop_HVDC_VSC_SC` instead.
:::
