...
description: HVDC two-port device models connecting two buses in RAMSES
---
...
<img src="/images/models/twop-hvdc-lcc-control-light.svg"
     alt="The HVDC_LCC control chain. Pset or Idset becomes a current order Ides, is filtered with time constant Tp to give Iord, and drives the rectifier's firing angle alpha, which sets the DC voltage Vdr. Vdr feeds the DC line resistance RL, giving Vdi at the inverter, which controls voltage or the extinction angle gamma."
     class="dark:sl-hidden" />
<img src="/images/models/twop-hvdc-lcc-control-dark.svg"
     alt="The HVDC_LCC control chain. Pset or Idset becomes a current order Ides, is filtered with time constant Tp to give Iord, and drives the rectifier's firing angle alpha, which sets the DC voltage Vdr. Vdr feeds the DC line resistance RL, giving Vdi at the inverter, which controls voltage or the extinction angle gamma."
...
<img src="/images/models/twop-hvdc-vsc-control-light.svg"
     alt="The HVDC_VSC control, one strip per axis. On the d axis Pref and Vdcref drive a P-V droop, an integrator of gain 1/Kid and a limiter between Idmin and Idmax to give idref, which the inner current PI turns into vmd using the feed-forward term vd plus omega L iq. On the q axis Qref and Vacref drive a Q-Vac droop and an integrator of gain 1/Kiq to give iqref1, which is summed with the FRT boost iqref2 to give iqref, and the inner current PI turns that into vmq using vq plus omega L id."
     class="dark:sl-hidden" />
<img src="/images/models/twop-hvdc-vsc-control-dark.svg"
     alt="The HVDC_VSC control, one strip per axis. On the d axis Pref and Vdcref drive a P-V droop, an integrator of gain 1/Kid and a limiter between Idmin and Idmax to give idref, which the inner current PI turns into vmd using the feed-forward term vd plus omega L iq. On the q axis Qref and Vacref drive a Q-Vac droop and an integrator of gain 1/Kiq to give iqref1, which is summed with the FRT boost iqref2 to give iqref, and the inner current PI turns that into vmq using vq plus omega L id."
     class="light:sl-hidden" />
...
### HVDC_VSC_SC (`twop_HVDC_VSC_SC`): VSC-HVDC with Self-Commutation (Grid-Forming)
...
<img src="/images/models/twop-hvdc-vsc-sc-control-light.svg"
     alt="The grid-forming HVDC_VSC_SC control. A frequency term Kwi times one minus omega1 and a power term delta P over Snom are summed into N1. N1 passes through an integrator of gain 1/Kip to give N1a, and in parallel through a proportional gain Kpp; the two are summed, limited, and become idref, which the inner current PI turns into vmd."
     class="dark:sl-hidden" />
<img src="/images/models/twop-hvdc-vsc-sc-control-dark.svg"
     alt="The grid-forming HVDC_VSC_SC control. A frequency term Kwi times one minus omega1 and a power term delta P over Snom are summed into N1. N1 passes through an integrator of gain 1/Kip to give N1a, and in parallel through a proportional gain Kpp; the two are summed, limited, and become idref, which the inner current PI turns into vmd."
     class="light:sl-hidden" />
...
<img src="/images/models/twop-dcl-vsc-topology-light.svg"
     alt="The DCL_VSC arrangement. A wind farm feeds bus 1 on the offshore AC side. Converter 1 is grid-forming, with a P-omega droop setting idref from P0, delta P and Kwi times delta omega, and a QV droop setting iqref. A DC cable with resistance Rdc and inertias Hdc1 and Hdc2 carries the power to converter 2, which is grid-following, controls DC voltage to set idref and uses a Q-Vac droop for iqref, and feeds bus 2 on the onshore AC grid."
     class="dark:sl-hidden" />
<img src="/images/models/twop-dcl-vsc-topology-dark.svg"
     alt="The DCL_VSC arrangement. A wind farm feeds bus 1 on the offshore AC side. Converter 1 is grid-forming, with a P-omega droop setting idref from P0, delta P and Kwi times delta omega, and a QV droop setting iqref. A DC cable with resistance Rdc and inertias Hdc1 and Hdc2 carries the power to converter 2, which is grid-following, controls DC voltage to set idref and uses a Q-Vac droop for iqref, and feeds bus 2 on the onshore AC grid."
     class="light:sl-hidden" />
