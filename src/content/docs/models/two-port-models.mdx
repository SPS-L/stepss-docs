...
Three two-port models are available in every RAMSES distribution (both the standalone executable and the shared library used by stepss): `HVDC_LCC` / `twop_HVDC_LCC`, `DCL_WCL` / `twop_DCL_WCL`, and `HVDC_VSC_SC` / `twop_HVDC_VSC_SC`. `twop_HVDC_VSC`, also documented below, is compiled into the library but registered under no name; it becomes callable by adding one case to the URAMSES router (see the [URAMSES guide](/developer/uramses/)).
:::
