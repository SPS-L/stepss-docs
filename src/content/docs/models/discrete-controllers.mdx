...
All of the above are built into every RAMSES distribution (standalone executable and shared library used by PyRAMSES). `MAIS` is a generic AGC-style integrator that does not have a dedicated documentation section below. Additional variants documented below but not in this list (`dctl_hvdc_lim`, `dctl_injprot`, `dctl_losprot`, and the Hydro-Québec family) are not callable out of the box. They can be enabled by extending RAMSES through URAMSES (see the [URAMSES guide](/developer/uramses/)).
:::
