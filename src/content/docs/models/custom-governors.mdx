...
It also documents `tor_gasturbm`, `tor_govclasm`, `tor_govhydr` and `tor_govnuc`, which are compiled but **registered under no name**, so RAMSES rejects them in a data file. Enabling one means adding a case to the URAMSES router (see the [URAMSES guide](/developer/uramses/)).
...
## tor_gasturbm (not registered)

:::caution
`tor_gasturbm` is documented here for reference but is **not callable out of the box** in a standard RAMSES distribution. To use it, enable it via URAMSES (see the [URAMSES guide](/developer/uramses/)).
:::
...
## tor_govclasm (not registered)

:::caution
`tor_govclasm` is documented here for reference but is **not callable out of the box** in a standard RAMSES distribution. To use it, enable it via URAMSES (see the [URAMSES guide](/developer/uramses/)).
:::
...
## tor_govhydr (not registered)

:::caution
`tor_govhydr` is documented here for reference but is **not callable out of the box** in a standard RAMSES distribution. To use it, enable it via URAMSES (see the [URAMSES guide](/developer/uramses/)).
:::
...
## tor_govnuc (not registered)

:::caution
`tor_govnuc` is documented here for reference but is **not callable out of the box** in a standard RAMSES distribution. To use it, enable it via URAMSES (see the [URAMSES guide](/developer/uramses/)).
:::
