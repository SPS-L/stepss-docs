...
All of the above are built into every RAMSES distribution (standalone executable and shared library used by PyRAMSES). Additional variants documented below but not in this list (`inj_INDM1`, `inj_norton`, `inj_PVG`) are not callable out of the box. They can be enabled by extending RAMSES through URAMSES (see the [URAMSES guide](/developer/uramses/)).
...
The model was contributed by Thierry Van Cutsem (July 2026). The block diagrams below are reproduced from the companion notes shipped with the model (`models/inj/custom/Grid_forming_model.pptx` in the RAMSES repository).
...
Default values are those of the `Benchmark_1_VSC` test case shipped with RAMSES (`examples/Benchmark_1_VSC`, converter of 1100 MVA). Per-unit values refer to the nominal apparent power of the converter.
...
The `Benchmark_1_VSC` case (grid-forming converter against a Thévenin equivalent):
...
The model was contributed by Thierry Van Cutsem (July 2026). The block diagrams below are reproduced from the companion notes shipped with the model (`models/inj/custom/Grid_following_model.pptx` in the RAMSES repository).
...
Default values are those of the `Benchmark_1_VSC` test case shipped with RAMSES (`examples/Benchmark_1_VSC`, converter of 1200 MVA). Per-unit values refer to the nominal apparent power of the converter.
...
Setting `Vs1` and `Vs2` to large negative values (as in the benchmark) disables the dynamic voltage support, following the recommendation in the companion notes that a more elaborate control should be considered for that function. Typical values of `Kiv` are 50 pu/s for voltage control and 10 pu/s for reactive power control.
...
The `Benchmark_1_VSC` case (grid-following converter against a Thévenin equivalent):
