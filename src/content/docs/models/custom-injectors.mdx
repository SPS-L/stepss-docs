...
- **Prefixed names** - RAMSES adds the `inj_` prefix automatically: `VFAULT`/`inj_VFAULT`, `vfd_load`/`inj_vfd_load`, `PQ`/`inj_PQ`, `IBG`/`inj_IBG`, `WT3`/`inj_WT3`, `WT4`/`inj_WT4`, `BESS`/`inj_BESS`, `GFOL`/`inj_GFOL`, `GFOR`/`inj_GFOR`.

All of the above are built into every RAMSES distribution (standalone executable and shared library used by PyRAMSES). Additional variants documented below but not in this list (`inj_GFOR_v2`, `inj_INDM1`, `inj_norton`, `inj_PVG`, `inj_PV`) are not callable out of the box - they can be enabled by extending RAMSES through URAMSES (see the [URAMSES guide](/developer/uramses/)).
...
### `WT3` (`inj_WT3WithChanges`) - Type 3 Wind Turbine (DFIG)
...
### `WT4` (`inj_WT4WithChanges`) - Type 4 Wind Turbine (Full Converter)
...
:::caution
`inj_PVG` is documented here for reference but is not callable out of the box in a standard RAMSES distribution. To use it, enable it via URAMSES (see the [URAMSES guide](/developer/uramses/)). For built-in IBR modelling, use `IBG`, `WT3`, or `WT4`.
:::
...
The data-file model name is `GFOR` (or `inj_GFOR`). `GFOR` resolves to the original grid-forming converter described below. A newer alternative implementation (described in the next section) is not dispatched by default.
...
:::caution
`inj_GFOR_v2` is an alternative grid-forming converter implementation documented here for reference. It is **not** the model dispatched by `GFOR` in a standard RAMSES distribution (which uses the implementation shown in the previous section). To use this newer variant, enable it via URAMSES (see the [URAMSES guide](/developer/uramses/)).
:::
...
### `BESS` (`inj_BESSWithChanges`) - Battery Energy Storage System
