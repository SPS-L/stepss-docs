...
RAMSES adds the `tor_` prefix to the model name automatically, so both `DEGOV1` and `tor_DEGOV1` resolve to the same model. The IEEE governor names available in every RAMSES distribution are:

| Data-file name | Equivalent |
|----------------|-----------|
| `DEGOV1` | `tor_DEGOV1` |
...
The data-file model name is `TGOV1` (or `tor_TGOV1` - the `tor_` prefix is added automatically). Although this model is internally known as `tor_TGOV1D`, the name registered for use in data files is `TGOV1` (without the `D`).
...
The data-file model name is `HYGOV` (or `tor_HYGOV` - the `tor_` prefix is added automatically).
