...
This page documents the exciters that are not IEEE-standard models: `CONSTANT`, `1ST_ORDER`, `GENERIC1` and `GENERIC2` (built in, uppercase, no prefix), plus `kundur` and `GENERIC` (registered, and RAMSES adds the `exc_` prefix automatically).
