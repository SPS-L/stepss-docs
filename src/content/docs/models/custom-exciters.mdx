...
All of the above are built into every RAMSES distribution (the standalone executable and the shared library used by PyRAMSES). Additional IEEE variants that appear in the catalogue below but are not in this list (e.g. `AC1A_MAXEX2`, `AC8B`, `DC3A`, `EXHQSC*`, `ST1A_PSS3B`, `ST2A`, `SEXS_STAB3_lim`, etc.) are not callable out of the box — they can be enabled by extending RAMSES through URAMSES (see the [URAMSES guide](/developer/uramses/)).
:::
