...
This page documents the exciters that are not IEEE-standard models: `CONSTANT`, `1ST_ORDER`, `GENERIC1` and `GENERIC2` (built in, uppercase, no prefix), plus `kundur`, `GENERIC` and `AVR_DG` (registered, and RAMSES adds the `exc_` prefix automatically).

The IEEE-standard exciters are on the [IEEE Exciter Models](/models/ieee-exciters/) page. For the whole catalogue at a glance, including which models are compiled but not callable by name, see the [Model Reference index](/models/).
:::

---

## CONSTANT (`exc_constant`)
...
<img src="/images/models/exc-1storder-light.svg"
     alt="1ST_ORDER exciter block diagram. The terminal voltage V is subtracted from the reference Vref and the error drives a first-order automatic voltage regulator of gain KA and time constant TA, limited between Vfmin and Vfmax, giving the field voltage vf. Vref is initialised as V0 plus Vf0 over KA."
     class="dark:sl-hidden" />
<img src="/images/models/exc-1storder-dark.svg"
     alt="1ST_ORDER exciter block diagram. The terminal voltage V is subtracted from the reference Vref and the error drives a first-order automatic voltage regulator of gain KA and time constant TA, limited between Vfmin and Vfmax, giving the field voltage vf. Vref is initialised as V0 plus Vf0 over KA."
...
<img src="/images/models/exc-kundur-light.svg"
     alt="exc_kundur block diagram. The terminal voltage passes a transducer of time constant TR to give Vfil, which is subtracted from Vref and summed with the stabiliser output Vs. That error drives an automatic voltage regulator of gain KA with transient gain reduction, a lead-lag with zero TA and pole TB, giving the field voltage vf. The stabiliser takes rotor speed through a washout of gain KSTAB and time constant TW and two lead-lag stages with time constants T1 to T4."
     class="dark:sl-hidden" />
<img src="/images/models/exc-kundur-dark.svg"
     alt="exc_kundur block diagram. The terminal voltage passes a transducer of time constant TR to give Vfil, which is subtracted from Vref and summed with the stabiliser output Vs. That error drives an automatic voltage regulator of gain KA with transient gain reduction, a lead-lag with zero TA and pole TB, giving the field voltage vf. The stabiliser takes rotor speed through a washout of gain KSTAB and time constant TW and two lead-lag stages with time constants T1 to T4."
     class="light:sl-hidden" />
...
## GENERIC1 (`exc_generic1`)

The data-file model name is `GENERIC1`.

<img src="/images/models/exc-generic1-light.svg"
     alt="exc_generic1 block diagram. The terminal voltage is subtracted from Vref, summed with the stabiliser signal and the over-excitation limiter output, and the error drives an AVR of gain G with a lead-lag of TA and TB and an exciter lag TE limited between L3 and L4. The stabiliser takes speed or active power through a washout of gain KPSS and two lead-lag stages, and an inverse-time OEL acts on the field current."
     class="dark:sl-hidden" />
<img src="/images/models/exc-generic1-dark.svg"
     alt="exc_generic1 block diagram. The terminal voltage is subtracted from Vref, summed with the stabiliser signal and the over-excitation limiter output, and the error drives an AVR of gain G with a lead-lag of TA and TB and an exciter lag TE limited between L3 and L4. The stabiliser takes speed or active power through a washout of gain KPSS and two lead-lag stages, and an inverse-time OEL acts on the field current."
     class="light:sl-hidden" />
...
<img src="/images/models/exc-avr-dg-light.svg"
     alt="exc_AVR_DG block diagram. The excitation system, stabiliser and over-excitation limiter are those of GENERIC1. What is added is a reactive-power PI regulator with gains KQP and KQI acting on the deviation from the reactive-power setpoint, whose output is a third contribution to the same summing junction, so the unit holds a reactive-power setpoint."
     class="dark:sl-hidden" />
<img src="/images/models/exc-avr-dg-dark.svg"
     alt="exc_AVR_DG block diagram. The excitation system, stabiliser and over-excitation limiter are those of GENERIC1. What is added is a reactive-power PI regulator with gains KQP and KQI acting on the deviation from the reactive-power setpoint, whose output is a third contribution to the same summing junction, so the unit holds a reactive-power setpoint."
     class="light:sl-hidden" />

### Scientific Description

An AVR for **dispersed generation**: the [`GENERIC1`](#generic1-exc_generic1) excitation
system, OEL and PSS, plus a **reactive-power PI controller** that trims the voltage
reference so the unit holds a reactive-power setpoint.

Parameters 1 to 23 are those of `GENERIC1`, in the same positions and with the same
...
model reduces to `GENERIC1`.

**Parameters:**

Indices 1 to 23 are exactly those of [`GENERIC1`](#generic1-exc_generic1). The four that
follow are new:
...
- `nbxvar = 8`, `nbzvar = 7`, `nbdata = 27`, `nbaddpar = 2`
- Additional: `prm(28) = V0` (voltage reference) and `prm(29) = Qset`, both captured at
  initialisation
- Observables: those of `GENERIC1` plus `deltavq` and `Qset`
...
<TabItem label="Notes">
- The setpoint is **not** a parameter. `Qset` is captured from the power-flow solution at
  initialisation, so the unit holds whatever reactive power it was dispatched at.
- With `KQP` and `KQI` non-zero the unit regulates reactive power rather than terminal
...
- `KPSS = 0` disables the stabiliser. Note that this drives the PSS branch to its lower
  limit, so `DVMIN` should be 0 as well, as in the example above.
- Pair it with [`HYDRO_DG`](/models/custom-governors/#hydro_dg-tor_hydro_dg) for a unit that
...
## GENERIC2 (`exc_generic2`)

The data-file model name is `GENERIC2`.

<img src="/images/models/exc-generic2-light.svg"
     alt="exc_generic2 block diagram. The terminal voltage is compensated for line drop with Xc and Rc, subtracted from Vref and summed with the stabiliser and over-excitation limiter signals. The error drives a combined proportional, integral and transient-gain-reduction regulator, an amplifier of gain KA and time constant TA, and an AC exciter of time constant TE with the saturation term SE."
     class="dark:sl-hidden" />
<img src="/images/models/exc-generic2-dark.svg"
     alt="exc_generic2 block diagram. The terminal voltage is compensated for line drop with Xc and Rc, subtracted from Vref and summed with the stabiliser and over-excitation limiter signals. The error drives a combined proportional, integral and transient-gain-reduction regulator, an amplifier of gain KA and time constant TA, and an AC exciter of time constant TE with the saturation term SE."
     class="light:sl-hidden" />
...
The data-file model name is `exc_GENERIC` (or `GENERIC`. The `exc_` prefix is added automatically).

:::caution[Renamed in RAMSES 3.57]
This model was called `exc_GENERIC3` up to and including 3.56, and a second
registered name, `exc_GENERIC4`, resolved to a byte-for-byte copy of it. Both
names were removed in 3.57 in favour of the single name `exc_GENERIC`.

On 3.57 and later, a record naming `exc_GENERIC3` or `exc_GENERIC4` stops with
`Excitation model not found in definition`. Change it to `exc_GENERIC`; the 20
data parameters and their order are unchanged, so nothing else in the record
moves.
:::

<img src="/images/models/exc-generic-light.svg"
     alt="exc_GENERIC block diagram. The terminal voltage is subtracted from V0, summed with the stabiliser signal and less the over-excitation limiter output, and the error drives a transient-gain-reduction block of gain G with Ta and Tb and an exciter lag Te limited between Vfmin and Vfmax. The stabiliser takes rotor speed through a washout of gain KPSS and two lead-lag stages."
     class="dark:sl-hidden" />
<img src="/images/models/exc-generic-dark.svg"
     alt="exc_GENERIC block diagram. The terminal voltage is subtracted from V0, summed with the stabiliser signal and less the over-excitation limiter output, and the error drives a transient-gain-reduction block of gain G with Ta and Tb and an exciter lag Te limited between Vfmin and Vfmax. The stabiliser takes rotor speed through a washout of gain KPSS and two lead-lag stages."
     class="light:sl-hidden" />
...
- [Model Reference](/models/), the full model catalogue
- [IEEE Exciters](/models/ieee-exciters/), the IEEE-standard excitation models
- [Dynamic Data Records](/user-guide/dynamic-models/), the `SYNC_MACH` record these models sit in
