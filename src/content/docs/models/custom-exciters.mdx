...
Recognised exciter model names (case-sensitive):

- **Uppercase short names** (no prefix): `CONSTANT`, `1ST_ORDER`, `GENERIC1`, `GENERIC2`.
- **Prefixed names**: RAMSES adds the `exc_` prefix automatically, so you can write either form: `kundur`/`exc_kundur`, `ENTSOE_simp`, `ST1A`, `SEXS`, `SEXS_IEEEST`, `GENERIC3`/`exc_GENERIC3`, `GENERIC4`/`exc_GENERIC4`, `AC1A`, `AC4A`, `IEEET5`, `ST1A_IEEEST`, `ST1A_PSS4B`, `ST1A_PSS2B`, `EXPIC1_PSS2B`.

All of the above are built into every RAMSES distribution (the standalone executable and the shared library used by PyRAMSES). The additional variants on the [IEEE Exciter Models](/models/ieee-exciters/) page (`AC1A_MAXEX2`, `AC8B`, `DC3A`, `ST1A_PSS3B`, `ST2A`, `SEXS_STAB3_lim` and the rest) are compiled but registered under no name, so they are not callable out of the box. Each becomes callable by adding one case to the URAMSES router (see the [URAMSES guide](/developer/uramses/)).
:::
