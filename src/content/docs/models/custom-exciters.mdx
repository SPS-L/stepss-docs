...
This page documents the exciters that are not IEEE-standard models: `CONSTANT`, `1ST_ORDER`, `GENERIC1` and `GENERIC2` (built in, uppercase, no prefix), plus `kundur`, `GENERIC3` and `GENERIC4` (registered, and RAMSES adds the `exc_` prefix automatically).
...
## GENERIC3 (`exc_GENERIC3`)

The data-file model name is `exc_GENERIC3` (or `GENERIC3`. The `exc_` prefix is added automatically).
...
SYNC_MACH GEN1  BUS1  1. 1.  0.  0.   600. 570. 3. 0. 0.95
                    XT   0.15  1.1  0.25  0.2  0.7  *  0.2  0.1  6.0257  0.  5.00  0.05  *  0.1
              EXC exc_GENERIC3  200.0  0.05  10.0  0.04  0.0  5.0
...
## GENERIC4 (`exc_GENERIC4`)

The data-file model name is `exc_GENERIC4` (or `GENERIC4`. The `exc_` prefix is added automatically).

### Scientific Description

`exc_GENERIC4` is a **second name for the same model as `exc_GENERIC3`**. Both are
CODEGEN-generated from the same description, and in the RAMSES source
`exc_GENERIC4.f90` is identical to `exc_GENERIC3.f90` apart from the subroutine
name. They are registered as separate cases in the exciter dispatcher, so both
names load, and they behave identically.

:::caution[There is no reason to prefer one over the other]
Earlier revisions of this page described `exc_GENERIC4` as a faster compiled
replacement for an interpreted `exc_GENERIC3`. That is not the case: CODEGEN
generates Fortran, which is compiled like any other model, and the two files are
byte-identical in their generated bodies. Expect no performance difference.
:::

The mathematical structure is therefore exactly that of `exc_GENERIC3`:
...
SYNC_MACH GEN1  BUS1  1. 1.  0.  0.   600. 570. 3. 0. 0.95
                    XT   0.15  1.1  0.25  0.2  0.7  *  0.2  0.1  6.0257  0.  5.00  0.05  *  0.1
              EXC exc_GENERIC4  200.0  0.05  10.0  0.04  0.0  5.0
                            ! G     Ta    Tb    Te    vfmin vfmax
                            9.5  1.40  0.154  0.033  0.154  0.033  0.10
                            ! Kpss Tw   T1     T2     T3     T4     C
                            1.05  0.30  30.0  1.0  0.0  1.0  2.0
...
<TabItem label="Notes">
- `exc_GENERIC4` and `exc_GENERIC3` are the same model under two names; pick either.
- Parameter order and meaning are identical to `exc_GENERIC3`.
- The model name in the data file is `exc_GENERIC4` (or `GENERIC4`. The `exc_` prefix is added automatically).
</TabItem>
</Tabs>
