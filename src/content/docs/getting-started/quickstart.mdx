...
| Interface | Dynamic Simulator (RAMSES) | Power Flow (Helios) | CODEGEN |
|-----------|:-------------------------:|:----------------------:|:-------:|
| **Command Line** | `ramses -t cmd.txt` | `helios -t cmd.txt` | `codegen -tmodel.txt` |
| **STEPSS GUI** | Via STEPSS GUI | Via STEPSS GUI | Via STEPSS GUI |
| **STEPSS in Python** | Via stepss | Via `stepss.helios` | |

The two editions are what you install; see
[Installation](/getting-started/installation/). The command line is not a third
thing to install, and the executables in that first row are not published on
their own: they ship inside STEPSS GUI, which unpacks them while it runs.
...
- [Python API](/python/api-reference/), Full Python API documentation
