...
# Fedora/RHEL
sudo dnf install gcc-gfortran make openblas-devel
...
```python
import pyramses
ram = pyramses.sim(custLibDir="path/to/Release_l")     # Linux
# ram = pyramses.sim(custLibDir="path/to/Release_m")   # macOS
# ram = pyramses.sim(custLibDir=r"path\to\Release_wg") # Windows (MinGW)
# ram = pyramses.sim(custLibDir=r"path\to\Release_wi") # Windows (Intel)
```
