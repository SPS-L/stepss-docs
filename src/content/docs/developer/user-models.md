...
where:
- $\mathbf{T}$ is a (mostly zero) matrix. Row $i$ is all zeros for algebraic equations, or contains a single $T_{ij}$ for differential equations (where $i$ and $j$ may be different). The nonzero components of $\mathbf{T}$ are time constants, typically.
...
At first glance, specifying both input and output states would lead to an overdetermined system with more equations than states. This is not the case: the excess equations are used to initialize the operating-point dependent parameters $\mathbf{u}$.
...
CODEGEN decomposes a model into a set of simple, interconnected **modelling blocks** (time constants, integrators, controllers, non-linearities, etc.). Each block contributes its algebraic and/or differential equations, which together form the complete DAE system.

Blocks are interconnected through **links**. A distinct internal state is associated with each link. Each of these states must be given by the user a unique name as well as an initial value.

Most blocks also involve their own internal states. Unlike the states associated with links between blocks, the user does not need to name or initialize these. This is done automatically by the code generated by CODEGEN.

Some modelling blocks also involve discrete variables $\mathbf{z}$.
...
The solver handles discrete transitions as follows:

1. For the current discrete variables $\mathbf{z}$, integrate from $t$ to $t+h$ with full accuracy
2. At the resulting point, check inequality constraints. If needed, update $\mathbf{z}$
3. Cancel the integration step, reset all states to their values at time $t$, and re-integrate from $t$ to $t+h$ with the new equations
4. Repeat steps 2-3 until no change in $\mathbf{z}$. If a limit cycle is detected (maximum number of changes reached), the time step is temporarily decreased to $h_{min}$. If the problem persists, the simulation stops and the model should be adjusted

See: D. Fabozzi, A. S. Chieh, P. Panciatici and T. Van Cutsem, "On simplified handling of state events in time-domain simulation," *Proc. 17th PSCC*, 2011.
