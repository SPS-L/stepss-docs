---
title: CODEGEN Model Examples
description: Complete CODEGEN model files for each model type — exciter, governor, injector, and two-port
---
...
**Source:** [SPS-L/stepss-ramses — IEEE_models/exc_ST1A.txt](https://github.com/SPS-L/stepss-ramses/blob/master/IEEE_models/exc_ST1A.txt)

<Tabs>
<TabItem label="Full Model File">
```
exc
ST1A
...
14. `algeq` × 2 — compute variable limits: $V_{RMIN} \cdot V_t$ and $V_{RMAX} \cdot V_t - K_C I_f$
15. `limvb` — bus-fed variable-limit output clamp → `vf`
...
**Source:** [SPS-L/stepss-ramses — IEEE_models/tor_hygov.txt](https://github.com/SPS-L/stepss-ramses/blob/master/IEEE_models/tor_hygov.txt)
...
hygov        ← model name (used as TOR hygov in the data file)
```
...
2. Filter: `tf1p` with time constant $T_f$
3. PI controller path: rate limiter (`lim`) → integrator (`inlim`) with gate limits → proportional bypass
4. Gate servo: `tf1p` with time constant $T_g$

**Turbine section:**
5. Head-flow relation: $\Delta H = 1 - (Q/g)^2$ (non-linear algebraic)
6. Water column inertia: $T_w \dot{Q} = \Delta H$ (`int` block)
...
**Source:** [SPS-L/stepss-ramses — custom_models/inj_vfd_load.txt](https://github.com/SPS-L/stepss-ramses/blob/master/custom_models/inj_vfd_load.txt)
...
1. `f_inj` — estimates bus frequency from voltage phasor derivatives
2. `algeq` — selects bus frequency or COI frequency based on `foption`

**Voltage processing:**
3. `algeq` — compute distribution bus voltage: $V_{real} = \sqrt{v_x^2 + v_y^2}/r$
4. `tf1p` — small delay (3 ms) for numerical stability
5. `pwlin4` — switch: $u = 1$ if $V > V_{min}$, $u = 0$ otherwise
...
**Source:** [SPS-L/stepss-ramses — custom_models/twop_HVDC_LCC.txt](https://github.com/SPS-L/stepss-ramses/blob/master/custom_models/twop_HVDC_LCC.txt)
...
HVDC_LCC     ← model name (used as TWOPOR ... HVDC_LCC in data file)
```
