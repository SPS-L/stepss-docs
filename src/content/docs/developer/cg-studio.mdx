...
Each model type exposes a different set of RAMSES input variables. For instance, an EXC model can reference `[v]`, `[p]`, `[q]`, `[omega]`, `[if]`, and `[vf]`, while a TOR model only has `[p]`, `[omega]`, and `[tm]`. See [User-Defined Models](/developer/user-models/) for the complete list.
:::
...
The browser downloads a `.json` file named `{model_name}.json` containing the complete project state:
...
A toast notification confirms: *"Project loaded: {model_name}"*.
...
The downloaded `.f90` file is named `{type}_{name}.f90` (e.g., `exc_ieee_st1a.f90`) and is ready to compile and link with RAMSES.
