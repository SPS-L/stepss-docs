...
A quantity may be followed by extra gnuplot commands, separated by `/`. These
are passed through into the `.plt` script the engine writes, for anyone who
opens that file in gnuplot themselves. They have no effect on the curves STEPSS
draws, which are not produced by gnuplot.

Live plotting needs nothing installed; see
[First Run](/gui/first-run/#real-time-plotting-needs-nothing-installed).
