...
5. Answer **Choose a directory to keep examples in**. This is asked once per
   session rather than once ever, because the choice is not remembered between
   launches. Any directory you can write to will do, and the same one every
   time keeps your cases together.
...
Two directories are kept separately, and the distinction matters once you open
more than one example.

- The **examples directory** is where copies are unpacked. You are asked for it
  the first time you open an example, and it is kept for as long as the
  application stays open. Opening a second example puts it beside the first
  rather than inside it.
- The **working directory** is where a run reads and writes. Opening an example
  moves it into that example.

Neither survives quitting, and neither is set from a menu: opening an example
is what chooses them. The banner that reports where the copy landed has an
**Open folder** button for getting at the files directly.
